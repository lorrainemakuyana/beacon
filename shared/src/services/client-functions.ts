 ...
 * These replace Cloud Functions to ensure 100% free tier Firebase usage
 */
...
export const joinShift = async (eventId: string, shiftId: string) => {
  ...
  console.log(`User ${auth.currentUser.uid} joining shift ${shiftId} in event ${eventId}`);
...
export const leaveShift = async (eventId: string, shiftId: string) => {
  ...
  console.log(`User ${auth.currentUser.uid} leaving shift ${shiftId} in event ${eventId}`);
...
export const checkEventPermission = async (eventId: string): Promise<boolean> => {
  ...
  return eventData.coordinators.includes(auth.currentUser.uid) ||
